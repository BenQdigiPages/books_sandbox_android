...
console.log('epub.js{generateRangeFromCfi}') ;	
	return range;
};
...
EPUBJS.Parser.prototype.container = function(containerXml){
		if(processFlag) console.log('*/{EPUBJS/Parser/getText|containerXml}[5403]');
		if(processFlag) console.log(containerXml);
...
EPUBJS.Parser.prototype.packageContents = function(packageXml, baseUrl){
	if(processFlag) console.log('*/{EPUBJS/Parser/packageContents|packageXml}[5451]');
	if(processFlag) console.log(packageXml);
...
EPUBJS.Unarchiver.prototype.getXml = function(url, encoding){
	if(processFlag) console.log('*/{Book/Unarchiver|S}[7674]');
	if(processFlag) console.log('*/{Book/Unarchiver|url}[7675]'+url);
	if(processFlag) console.log('*/{Book/Unarchiver|encoding}[7675]'+encoding);
	return this.getText(url, encoding).
			then(function(text){
				var parser = new DOMParser();
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|Have UTF-8 BOM}[7680]: ' + text.indexOf('\uEFBBBF'));
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|Have UTF-16LE BOM}[7681]: ' + text.indexOf('\uFEFF'));
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|Have UTF-16BE BOM}[7682]: ' + text.indexOf('\uFFFE'));
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|Have UTF-32LE BOM}[7683]: ' + text.indexOf('\u0000FEFF'));
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|Have UTF-32BE BOM}[7684]: ' + text.indexOf('\uFFFE0000'));
				if(processFlag) console.log('*/{Book/Unarchiver/getXml|hex}[7685]');
				if(processFlag) console.log(text.hexEncode());
				//if(processFlag) console.log(parser.parseFromString(text, "text/xml"));
...
EPUBJS.Unarchiver.prototype.getText = function(url, encoding){
	...
	if(processFlag) console.log('*/{Book/Unarchiver/getText|S}[7713]');
	...
	text = entry.asText();
	deferred.resolve(text);
	if(processFlag) console.log('*/{Book/Unarchiver/getText|text}[7730]' + text);
